...
{
	// add on the persistence indicator
	headers.push_back(header());
	if (!persistent)
	{
		headers.back().name = "Connection";
		headers.back().value = "close";
	}
	else
	{
		headers.back().name = "Connection";
		headers.back().value = "Keep-Alive";

		headers.push_back(header());
		headers.back().name = "Keep-Alive";
		headers.back().value = "timeout=30";
	}
	
	// and the content length
	headers.push_back(header());
	headers.back().name = "Content-Length";
	headers.back().value = boost::lexical_cast<std::string>(content.size());

	// ok, now setup the buffers for the request
...
reply reply::stock_reply(reply::status_type status, bool prefers_persistent)
{
	reply rep;
	rep.status = status;
