...
static
WebDMA * m_instance = NULL;

// this ensures that the singleton WebDMA gets deleted at exit
StaticDeleter<WebDMA> m_deleter(&m_instance);
...
/// this is the thread that the HTTP server runs on. When the WebDMA singleton
...
void WebDMA::EnableInternal(const std::string &port, const std::string &rootdir)
{
	lock_guard lock(m_mutex);
	...
	if (!m_thread_created)
	{
		m_thread.reset( new boost::thread(boost::bind(&WebDMA::ThreadFn, this)) );
		m_thread_created = true;
	}
}

// internal function
WebDMA * WebDMA::GetInstance()	
{
	if (m_instance == NULL)
		m_instance = new WebDMA();
	return m_instance;
}


/// this function returns the html to be inserted into the index.html page
std::string WebDMA::get_html()
{
	// lock globally
	lock_guard lock(m_mutex);
...
*/
std::string WebDMA::ProcessRequest(const std::string &post_data)
{
